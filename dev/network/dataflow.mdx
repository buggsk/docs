...
description: ''
---
The role of Shipper starts the flow of data into the Orchestro Network. A Brand or a Carrier Participant can act as a Shipper by submitting an order to the network. A Participant Carrier in the role of the Destination Carrier, receives data from the Orchestro network and pushes events to Orchestro completing the package data lifecycle to the destination delivery. 
For a deeper understanding, please find below the journey of the parcel from origin to destination in a multi-party network. 
...
| 2 | Orchestro | Destination Carrier | Orchestro acts as carrier and receives manifest from Origin carrier/Brand All tracking info |- All tracking info | Async error messages |
...
## Manifest Data Feed 
The manifest data feed will send the information in the format as defined in the Standard Manifest section below. 

## Tracking Data Feed 
The tracking data feed will send the information in the format as defined in the Standard Event Format section below. 

## Error Data Feed 
Most errors encountered in the Orchestro system are asynchronous and could be directly due to an input from the carrier system or could be due to an event received from the network. Errors are published to all affected parties in real time, and carriers are expected to consume and process these errors. 
